...
class ContratsDAL:
    ...
    def valider_contrat_transaction(self, id_client, date_debut, date_fin_prevue, 
                                    article_ids, prix_final, lignes):       # Valider contrat (transaction) atomique - lock articles (FOR UPDATE) -check dispo  - insert contrat + lignes - maj statut articles      
        conn = get_connection()
        if not conn:
        ...
        try:
            ...
            with conn.cursor() as cur:      # transaction atomique
                if not article_ids:
                    conn.rollback()
                ...
                if len(rows) != len(article_ids):   # check tous trouvés
                    conn.rollback()
    ...
    def client_a_location_en_retard(self, id_client):      # check si ligne en retard    
        conn = get_connection()
        if not conn:
            return False
